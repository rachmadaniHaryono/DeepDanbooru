...
def test_package_setup(packages):
    ...
    assert list(filter(lambda x: x.startswith("tensorflow"), packages)) == [
        tensorflow_pkg
    ]
...
def test_readme_pkg(packages):
    ...
    ) == list(sorted(filter(lambda x: not x.startswith("tensorflow"), packages)))

    assert list(filter(lambda x: x.startswith("tensorflow"), packages)) == list(
